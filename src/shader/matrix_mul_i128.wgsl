...
// Constructors
fn own_i64_from_i32(_n: i32) -> own_i64 {
    var r = own_i64();
    r.number[1] = u32(abs(_n));
    r.sign = select(1, -1, _n < 0);
    return r;
}
fn own_i64_from_u32(_n: u32) -> own_i64 {
    var r = own_i64();
    r.number[1] = _n;
    r.sign = 1;
    return r;
}
fn own_i64_from_u32_array(_number: array<u32, 2>, _sign: i32) -> own_i64 {
    var r = own_i64();
    r.number = _number;
    r.sign   = _sign;
    return r;
...
fn i128_from_own_i64(_n: own_i64) -> i128 {
    var r = i128();
    var arr = array<u32, 4>();
...
// Unary operators
fn own_i64_abs(_n: own_i64) -> own_i64 {
    var r = _n;
    r.sign = 1;
    return r;
}
fn own_i64_negate(_n: own_i64) -> own_i64 {
    var r = _n;
    r.sign = -1;
    return r;
}
fn own_i64_opposite(_n: own_i64) -> own_i64 {
    var r = _n;
...
// Logic operators
fn own_i64_and(_a: own_i64, _b: own_i64) -> own_i64 {
    var r = own_i64_from_u32(0);
    r.sign = 1;
    for (var i = u32(0); i != 2; i++) {
...
fn own_i64_or(_a: own_i64, _b: own_i64) -> own_i64 {
    var r = own_i64_from_u32(0);
    r.sign = 1;
    for (var i = u32(0); i != 2; i++) {
    	r.number[i] = _a.number[i] | _b.number[i];
...
fn own_i64_xor(_a: own_i64, _b: own_i64) -> own_i64 {
    var r = own_i64_from_u32(0);
    r.sign = 1;
    for (var i = u32(0); i != 2; i++) {
    	r.number[i] = _a.number[i] ^ _b.number[i];
...
fn own_i64_left_shift(_n: own_i64, _s: u32) -> own_i64 {
    var r = _n;
...
fn own_i64_right_shift(_n: own_i64, _s: u32) -> own_i64 {
    var r = _n;
    var shiftLoops    = u32(floor(f32(_s) / 31));
    var shiftLastLoop = u32(_s % 31);
    for (var i = u32(0); i != shiftLoops; i++) {
...
fn own_i64_eq(_a: own_i64, _b: own_i64) -> bool {
    var is_zero = true;
    for (var i = u32(0); i != 2; i++) {
...
fn own_i64_greater(_a: own_i64, _b: own_i64) -> bool {
    for (var i = u32(0); i != 2; i++) {
...
fn own_i64_greater_eq(_a: own_i64, _b: own_i64) -> bool {
    if (own_i64_greater(_a, _b) || own_i64_eq(_a, _b)) {
        return true;
    }
    return false;
}
fn own_i64_less(_a: own_i64, _b: own_i64) -> bool {
    if (!own_i64_greater_eq(_a, _b)) {
        return true;
    }
    return false;
}
fn own_i64_less_eq(_a: own_i64, _b: own_i64) -> bool {
    if (!own_i64_greater(_a, _b)) {
        return true;
    }
    return false;
}
...
fn own_i64_max(_a: own_i64, _b: own_i64) -> own_i64 {
    if (own_i64_greater(_a, _b)) {
        return _a;
    }
    return _b;
}
fn own_i64_min(_a: own_i64, _b: own_i64) -> own_i64 {
    if (own_i64_less(_a, _b)) {
        return _a;
    }
    return _b;
}
...
fn own_i64_sum(_a: own_i64, _b: own_i64) -> own_i64 {
    var first  = _a;
    var second = _b;
    if (own_i64_less(own_i64_abs(first), own_i64_abs(second))) {
        var temp = first;
        first  = second;
        second = temp;
    }
    if (first.sign == -1 && second.sign == -1) {
        return own_i64_negate(summation64(own_i64_abs(first), own_i64_abs(second)));
    }
    if (first.sign == 1 && second.sign == -1) {
        return subtraction64(first, own_i64_abs(second));
    }
    if (first.sign == -1 && second.sign == 1) {
        return own_i64_negate(subtraction64(own_i64_abs(first), second));
    }
    return summation64(first, second);
}
fn own_i64_sub(_a: own_i64, _b: own_i64) -> own_i64 {
    var first    = _a;
    var second   = _b;
    var opposite = u32(0);
    if (own_i64_less(own_i64_abs(first), own_i64_abs(second))) {
    ...
    if (first.sign == -1 && second.sign == -1) {
        if (opposite == 1) {
            return own_i64_opposite(own_i64_negate(subtraction64(own_i64_abs(first), own_i64_abs(second))));
        }
        else {
            return own_i64_negate(subtraction64(own_i64_abs(first), own_i64_abs(second)));
        }
    }
    if (first.sign == 1 && second.sign == -1) {
        if (opposite == 1) {
            return own_i64_opposite(summation64(first, own_i64_abs(second)));
        }
        else {
            return summation64(first, own_i64_abs(second));
        }
    }
    if (first.sign == -1 && second.sign == 1) {
        if (opposite == 1) {
            return own_i64_opposite(own_i64_negate(summation64(own_i64_abs(first), second)));
        }
        else {
            return own_i64_negate(summation64(own_i64_abs(first), second));
        }
    }
    if (opposite == 1) {
        return own_i64_opposite(subtraction64(first, second));
    }
    else {
...
fn own_i64_mul_to_own_i64(_a: own_i64, _b: own_i64) -> own_i64 {
    var r_temp = karatsuba64(_a, _b);
    var arr = array<u32, 2>();
    for (var i = u32(0); i != 2; i++) {
        arr[1 - i] = r_temp.number[3 - i];
    }
    return own_i64_from_u32_array(arr, _a.sign * _b.sign);
}
fn own_i64_mul_to_i128(_a: own_i64, _b: own_i64) -> i128 {
    var r = karatsuba64(_a, _b);
    r.sign = _a.sign * _b.sign;
    return r;
}
fn own_i64_div(_a: own_i64, _b: own_i64) -> array<own_i64, 2> {
    let a    = own_i64_abs(_a);
    let b    = own_i64_abs(_b);
    let zero = own_i64_from_u32(0);
    let one  = own_i64_from_u32(1);
    if (own_i64_eq(b, zero)) {
        return array<own_i64, 2>(own_i64_from_u32_array(array<u32, 2>(), 1), own_i64_from_u32_array(array<u32, 2>(), 1));
    }
    else if (own_i64_eq(b, one)) {
        return array<own_i64, 2>(a, zero);
    }
    else if (own_i64_eq(a, b)) {
        return array<own_i64, 2>(one, zero);
    }
    else if ((own_i64_eq(a, zero)) || (own_i64_less(a, b))) {
        return array<own_i64, 2>(zero, a);
    }
    var qr = array<own_i64, 2>(zero, zero);
    for (var x = u32(64); x > 0; x--) {
        qr[0] = own_i64_left_shift(qr[0], 1);
        qr[1] = own_i64_left_shift(qr[1], 1);
        if ((own_i64_right_shift(a, x - 1).number[1] & 1) == 1) {
            qr[1] = own_i64_sum(qr[1], one);
        }
        if (own_i64_greater_eq(qr[1], b)) {
            qr[1] = own_i64_sub(qr[1], b);
            qr[0] = own_i64_sum(qr[0], one);
        }
    }
...
// PowerMod operators
fn own_i64_powermod(_b: own_i64, _e: own_i64, _m: own_i64) -> own_i64 {
    if (own_i64_eq(_m, own_i64_from_u32(0))) {
        return own_i64_from_i32(-1);
    }
    if (own_i64_eq(_m, own_i64_from_u32(1))) {
        return own_i64_from_u32(0);
    }
    var base     = _b;
    var exponent = _e;
    var result   = own_i64_from_i32(1);
    base = own_i64_div(base, _m)[1];
    while (own_i64_greater(exponent, own_i64_from_u32(0))) {
        if (exponent.number[1] % 2 == 1) {
            result = own_i64_div(own_i64_mul_to_own_i64(result, base), _m)[1];
        }
        exponent = own_i64_right_shift(exponent, 1);
        base     = own_i64_div(own_i64_mul_to_own_i64(base, base), _m)[1];
    }
    return result;
}
...
// Auxiliary functions
fn summation64(_a: own_i64, _b: own_i64) -> own_i64 {
    var r = own_i64_from_u32(0);
    r.sign = _a.sign;
    var overflow = u32(0);
    for (var i = u32(0); i != 2; i++) {
...
fn subtraction64(_a: own_i64, _b: own_i64) -> own_i64 {
    var r = own_i64_from_u32(0);
    r.sign = _a.sign;
    var underflow = u32(0);
    for (var i = u32(0); i != 2; i++) {
...
fn karatsuba32(_a: u32, _b: u32) -> own_i64 {
    let x_h = _a >> 16;
    ...
    let lh = x_l * y_h;
    let ll = x_l * y_l;
    return own_i64_sum(own_i64_sum(own_i64_left_shift(own_i64_from_u32(hh), 32), own_i64_left_shift(own_i64_sum(own_i64_from_u32(hl), own_i64_from_u32(lh)), 16)), own_i64_from_u32(ll));
}
fn karatsuba64(_a: own_i64, _b: own_i64) -> i128 {
    let x_h = _a.number[0];
    let x_l = _a.number[1];
    let y_h = _b.number[0];
    let y_l = _b.number[1];
    ...
    return i128_sum(i128_sum(i128_left_shift(i128_from_own_i64(hh), 64), i128_left_shift(i128_sum(i128_from_own_i64(hl), i128_from_own_i64(lh)), 32)), i128_from_own_i64(ll));
}
fn karatsuba128(_a: i128, _b: i128) -> i256 {
    ...
    let x_h = own_i64_from_u32_array(a_high, 1);
    let x_l = own_i64_from_u32_array(a_low, 1);
    let y_h = own_i64_from_u32_array(b_high, 1);
    let y_l = own_i64_from_u32_array(b_low, 1);
...
@group(0) @binding(0)
var<storage, read> A: array<i128>;

@group(0) @binding(1)
var<storage, read> B: array<i128>;

@group(0) @binding(2)
var<uniform> dims: Dimensions;

@group(0) @binding(3)
var<storage, read_write> C: array<i128>;
...
fn mul(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    var sum: i128 = i128_from_u32(0u);
    for (var k = 0u; k < m; k++) {
        sum = i128_sum(sum, i128_mul_to_i128(A[row * m + k], B[k * p + col]));
    }
